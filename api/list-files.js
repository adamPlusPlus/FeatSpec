...
export default async function handler(req, res) {
  ...
  try {
    // List all files in feat-spec/ prefix
    const { blobs } = await list({ prefix: 'feat-spec/' });

    // Filter to only .json files and format response
    const files = blobs
      .filter(blob => blob.pathname.endsWith('.json'))
      .map(blob => ({
        name: blob.pathname.replace('feat-spec/', ''),
        size: blob.size,
